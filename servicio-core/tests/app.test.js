...
function buildMockPrisma() {
  ...
  return {
    ...
    company: {
      ...
      create: async (args) => {
        calls.push(['company.create', args]);
        return { id: 'company-1', ...args.data, botConfig: null };
      },
      update: async (args) => {
        calls.push(['company.update', args]);
        return { id: 'company-1', ...args.data, botConfig: null };
      },
      upsert: async (args) => {
...
test('POST /api/procesar-mensaje devuelve payload y registra el mensaje inbound', async () => {
  ...
  try {
    ...
    assert.equal(body.interactionCount?.increment, 1);
    assert.equal(body.messageBody, 'Hola desde prueba');
    assert.equal(mockPrisma.calls[0][0], 'conversationSession.create');
  } finally {
...
test('POST /api/procesar-mensaje procesa comandos de administración bloquear', async () => {
  const mockPrisma = buildMockPrisma();
  const app = crearAplicacion(mockPrisma);
  const server = app.listen(0);
  ...
  try {
    ...
    assert.equal(response.status, 200);
    assert.equal(body.status, 'success');
    assert.equal(body.reply, 'Bot bloqueado permanentemente para este usuario.');
  } finally {
    await new Promise((resolve) => server.close(resolve));
  }
});

test('POST /api/procesar-mensaje acepta payload de n8n con commando y jid con espacios', async () => {
  const mockPrisma = buildMockPrisma();
  const app = crearAplicacion(mockPrisma);
  const server = app.listen(0);
...
test('POST /api/procesar-mensaje trata resetear como reinicio del contador y desbloqueo', async () => {
  const mockPrisma = buildMockPrisma();
  const app = crearAplicacion(mockPrisma);
  const server = app.listen(0);
  ...
  try {
    ...
    assert.equal(response.status, 200);
    assert.equal(body.status, 'success');
    assert.equal(body.reply, 'Interacciones reiniciadas. El bot está activo nuevamente.');
  } finally {
    await new Promise((resolve) => server.close(resolve));
  }
});

test('POST /api/procesar-mensaje acepta reset con solo remoteJid', async () => {
  const mockPrisma = buildMockPrisma();
  const app = crearAplicacion(mockPrisma);
  const server = app.listen(0);
